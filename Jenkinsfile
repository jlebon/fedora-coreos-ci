def pod, utils, prod, devel_prefix, src_config_url, src_config_ref, s3_bucket
node {
    checkout scm
    ...
    node('coreos-assembler') { container('coreos-assembler') {
        ...
        if (s3_bucket && utils.path_exists("/.aws")) {
          if (prod) {
            ...
            s3_builddir = "${s3_bucket}/prod/streams/${params.STREAM}"
          } else {
            ...
            s3_builddir = "${s3_bucket}/devel/streams/${devel_prefix}"
          }
        }
